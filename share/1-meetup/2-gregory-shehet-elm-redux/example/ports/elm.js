...
Elm.Main = Elm.Main || {};
Elm.Main.Ports = Elm.Main.Ports || {};
Elm.Main.Ports.make = function (_elm) {
   "use strict";
   _elm.Main = _elm.Main || {};
   _elm.Main.Ports = _elm.Main.Ports || {};
   if (_elm.Main.Ports.values) return _elm.Main.Ports.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var modelValue = Elm.Native.Port.make(_elm).inboundSignal("modelValue",
   "String",
   function (v) {
      return typeof v === "string" || typeof v === "object" && v instanceof String ? v : _U.badPort("a string",v);
   });
   var view = function (value) {    return A2($Html.div,_U.list([]),_U.list([$Html.text($Basics.toString(value))]));};
   ...
   var actions = A2($Signal.merge,inbox.signal,A2($Signal.map,Update,modelValue));
   var initialModel = 0;
   var model = A3($Signal.foldp,update,initialModel,actions);
   var modelChanges = Elm.Native.Port.make(_elm).outboundSignal("modelChanges",function (v) {    return v;},model);
   var main = A2($Signal.map,view,model);
   return _elm.Main.Ports.values = {_op: _op
